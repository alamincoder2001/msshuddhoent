...
<div class="row" id="customerPaymentReport">
	<div class="col-xs-12 col-md-12 col-lg-12" style="border-bottom:1px #ccc solid;">
		<div class="form-group">
			<label class="col-sm-1 control-label">Search For</label>
			<div class="col-md-1" style="width: 130px;">
				<select class="form-control" v-model="search_for" style="padding: 1px;" v-on:change="onChangeSearchFor">
					<option value="" selected>All</option>
					<option value="due">Sale Due Report</option>
					<option value="charge">PO_Charge Pay</option>
				</select>
			</div>
			<div class="form-group">
				<label class="col-sm-1 control-label no-padding-right"> Customer </label>
				<div class="col-sm-2">
					<v-select v-bind:options="customers" id="customer" v-model="selectedCustomer" label="display_name"></v-select>
				</div>
			</div>
				...
				<table class="table table-bordered">
					<thead>
						<tr>
							...
							<th style="text-align:center">PO Paid</th>
							<th style="text-align:center">Inv.Due</th>
							<th style="text-align:center">PO Charge Amount</th>
					...
					<tbody>
						...
						<tr v-for="payment in payments">
							...
							<td style="text-align:right;">{{ parseFloat(payment.POpaid).toFixed(2) }}</td>
							<td style="text-align:right;">{{ parseFloat(payment.due).toFixed(2) }}</td>
							<td style="text-align:right;">{{ parseFloat(payment.cpo_charge).toFixed(2) }}</td>
		...
		<div class="col-sm-12" style="display:none;" v-bind:style="{display: showPoTable ? '' : 'none'}">
			<a href="" style="margin: 7px 0;display:block;width:50px;" v-on:click.prevent="print">
				<i class="fa fa-print"></i> Print
			</a>
			<div class="table-responsive" id="PoreportTable">
				<table class="table table-bordered">
					<thead>
						<tr>
							<th style="text-align:center">Date</th>
					...
					<tbody>
						<tr v-for="payment in payments_po_charge">
							<td>{{ payment.CPayment_date }}</td>
							<td style="text-align:left;">{{ payment.description }}</td>
							<td style="text-align:right;">{{ parseFloat(payment.CPayment_po_charge_amount).toFixed(2) }}</td>
							<td style="text-align:right;">{{ '0.00'}}</td>
							<td style="text-align:right;">{{ parseFloat(payment.balance).toFixed(2) }}</td>
						</tr>
					</tbody>
					<tbody v-if="payments_po_charge.length == 0">
						<tr>
							<td colspan="8">No records found</td>
						</tr>
					</tbody>
		...
		new Vue({
			...
			methods: {
				onChangeSearchFor(event) {
					this.payments = [];
					this.payments_po_charge = [];
					this.showTable = false;
					this.showPoTable = false;
				},
				getCustomers() {
				...
				getReport() {
					...
					let data = {
						...
						salesDue: this.search_for =="due"?"salesdue":''
					}
					if (this.search_for == 'due') {
					...
					} else if(this.search_for == "charge") {
						axios.post('/get_customer_payment_po_charge', data).then(res => {
							this.payments_po_charge = res.data;
							this.showPoTable = true;
							this.showTable = false;
						})
					}else{
						axios.post('/get_customer_ledger', data).then(res => {
				...
				async print() {
					...
					<div class="container">
						<div class="row">
							<div class="col-xs-12">
								${this.search_for !='charge'? document.querySelector('#reportTable').innerHTML:document.querySelector('#PoreportTable').innerHTML}
							</div>
						</div>
					</div>
				`;
