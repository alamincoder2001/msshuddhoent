		...
		<div class="row" style="padding-top:10px;">
			<div class="col-md-6">
				...
				<table class="table table-bordered table-condensed">
					...
					<tbody style="display:none;" v-bind:style="{display: poChargeSales.length > 0 ? '' : 'none'}">
						<tr v-for="(po,index) in poChargeSales">
							<td>{{ index+1 }}</td>
							<td>{{ po.SaleMaster_InvoiceNo }}</td>
							<td>{{ po.SaleMaster_SaleDate }}</td>
							<td style="text-align: right;">{{ po.po_charge | decimal }}</td>
						</tr>
					</tbody>
					<tfoot>
						<tr style="font-weight:bold;">
							<td colspan="3" style="text-align:right">Total</td>
							<td style="text-align:right;">
								<span v-if="poChargeSales.length == 0">0.00</span>
								<span style="display:none;" v-bind:style="{display: poChargeSales.length > 0 ? '' : 'none'}">{{ totalPoChargeSales | decimal }}</span>
							</td>
						</tr>
					</tfoot>
				</table>
...
<script>
	new Vue({
		...
		computed: {
			...
			totalPoChargeSales() {
				return this.poChargeSales.reduce((prev, curr) => {
					return prev + parseFloat(curr.po_charge)
				}, 0).toFixed(2);
			},
			totalInitialLoan() {
		...
		methods: {
			...
			getPoChargeSales() {
				...
				axios.post('/get_po_charge', filter)
					.then(res => {
						this.poChargeSales = res.data;
					})
			},
			getLoanPayments() {
				let filter = {
