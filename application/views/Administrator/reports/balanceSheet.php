					...
					<table class="balancesheet-table">
						...
						<tbody>
							...
							<tr>
								...
								<td style="text-align:right;">{{ (parseFloat(totalReceivedFromCustomers) + parseFloat(totalPoReceivedFromCustomers) + parseFloat(totalPoPaidSales)) | decimal }}</td>
							</tr>
							<tr>
								<td>Cash Received</td>
...
<script>
	new Vue({
		...
		computed: {
			...
			totalPoReceivedFromCustomers() {
				return this.receivedFromCustomers.reduce((prev, curr) => {
					return prev + parseFloat(curr.CPayment_po_charge_amount)
				}, 0).toFixed(2);
			},
			...
			totalCashIn() {
				return parseFloat(this.totalSales) +
					parseFloat(this.totalPoPaidSales) +
					parseFloat(this.totalPoReceivedFromCustomers) +
