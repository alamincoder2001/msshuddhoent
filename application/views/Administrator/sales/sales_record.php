			...
			<form class="form-inline" id="searchForm" @submit.prevent="getSearchResult">
				<div class="form-group">
					...
					<select class="form-control" v-model="searchType" @change="onChangeSearchType">
						...
						<option value="transport_only">Only Transport</option>
					</select>
				</div>

				<div class="form-group" style="display:none;" v-bind:style="{display: searchType == 'vehicle' ? '' : 'none'}">
					<label>Vehicle</label>
					<v-select v-bind:options="vehicles" v-model="selectedVehicle" label="vehicle_no"></v-select>
				</div>
				...
				<div class="form-group" v-bind:style="{display: searchTypesForRecord.includes(searchType) && searchType != 'transport_only' ? '' : 'none'}">
	...
	<div class="row" style="margin-top:15px;display:none;" v-bind:style="{display: sales.length > 0 ? '' : 'none'}">
		...
		<div class="col-md-12">
			<div class="table-responsive" id="reportContent">
				...
				<table class="record-table" v-if="(searchTypesForRecord.includes(searchType)) && recordType == 'without_details'" style="display:none" v-bind:style="{display: (searchTypesForRecord.includes(searchType)) && recordType == 'without_details' && searchType != 'transport_only' ? '' : 'none'}">
					<thead>
						<tr>
							<th>Invoice No.</th>
							<th>Date</th>
					...
					<tbody>
						<tr v-for="sale in sales">
							...
							<td style="text-align:right;">{{ sale.po_charge }}</td>
					...
					<tfoot>
						<tr style="font-weight:bold;">
							...
							<td style="text-align:right;">{{ sales.reduce((prev, curr)=>{return prev + parseFloat(curr.po_charge)}, 0) }}</td>
				...
				<template v-if="searchTypesForDetails.includes(searchType)" style="display:none;" v-bind:style="{display: searchTypesForDetails.includes(searchType) && searchType != 'transport_only' ? '' : 'none'}">
				...
				<table class="record-table" v-if="searchType == 'transport_only'">
					<thead>
						<tr>
							<th>Invoice No.</th>
							<th>Date</th>
	...
	new Vue({
		...
		data() {
			return {
				...
				sales: [],
				searchTypesForRecord: ['', 'user', 'customer', 'employee', 'vehicle', 'transport_only'],
		...
		methods: {
			...
			getSalesRecord() {
				let filter = {
					...
					vehicleNo: this.selectedVehicle == null || this.selectedVehicle.vehicle_no == '' ? '' : this.selectedVehicle.vehicle_no,
