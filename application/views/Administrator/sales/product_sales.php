...
<div id="sales" class="row">
	<div class="col-xs-12 col-md-12 col-lg-12" style="border-bottom:1px #ccc solid;margin-bottom:5px;">
		...
		<div class="row">
			<div class="form-group">
				<label class="col-sm-1 control-label no-padding-right"> Vehicle No </label>
				<div class="col-sm-2">
					<input type="text" placeholder="Vehicle no" class="form-control" v-model="sales.vehicle_no" />
				</div>
			</div>
		</div>
	</div>
							...
							<div class="table-responsive">
								<table style="color:#000;margin-bottom: 0px;border-collapse: collapse;">
									...
									<tr>
										<td>
											<div class="form-group">
												<label class="col-xs-12 control-label no-padding-right">PO Charge</label>
												<div class="col-xs-12">
													<input type="number" class="form-control" v-model="sales.po_charge" v-on:input="calculateTotal" />
												</div>
											</div>
										</td>
									</tr>
	...
	new Vue({
		...
		methods: {
			...
			calculateTotal() {
				...
				this.sales.total = ((parseFloat(this.sales.subTotal) + parseFloat(this.sales.vat) + parseFloat(this.sales.transportCost) + parseFloat(this.sales.po_charge)) - parseFloat(this.sales.discount)).toFixed(2);
			...
			async getSales() {
				await axios.post('/get_sales', {
					...
					this.sales.vehicle_no = sales.vehicle_no;
					this.sales.po_charge = sales.po_charge;
