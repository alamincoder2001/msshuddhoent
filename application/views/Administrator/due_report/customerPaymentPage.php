			...
			<form @submit.prevent="saveCustomerPayment">
				<div class="row">
					<div class="col-md-5 col-md-offset-1">
						...
						<div class="form-group">
							<label class="col-md-4 control-label">Sales Due</label>
							<label class="col-md-1">:</label>
							<div class="col-md-7">
						...
						<div class="form-group">
							<label class="col-md-4 control-label">PO Due</label>
							<label class="col-md-1">:</label>
							<div class="col-md-7">
								<input type="text" class="form-control" v-model="payment.CPayment_Previous_due_Po" disabled>
							</div>
						</div>
					</div>
					...
					<div class="col-md-5">
						...
						<div class="form-group">
							<label class="col-md-4 control-label">Sales Payment</label>
							<label class="col-md-1">:</label>
							<div class="col-md-7">
								<input type="number" class="form-control" v-model="payment.CPayment_amount" required :disabled="payment.CPayment_previous_due > 0 ?false:true">
							</div>
						</div>
						<div class="form-group">
							<label class="col-md-4 control-label">PO Payment</label>
							<label class="col-md-1">:</label>
							<div class="col-md-7">
								<input type="number" class="form-control" v-model="payment.CPayment_po_charge_amount" :disabled="payment.CPayment_Previous_due_Po > 0 ?false:true">
							</div>
						</div>
						<div class="form-group">
							...
							<label class="col-md-1">:</label>
							<div class="col-md-7">
								<textarea class="form-control" v-model="payment.CPayment_notes" cols="30" rows="2"></textarea>
							</div>
						</div>
						<div class="form-group">
				...
				<datatable :columns="columns" :data="payments" :filter-by="filter" style="margin-bottom: 5px;">
					<template scope="{ row }">
						<tr>
							...
							<td>{{ row.CPayment_po_charge_amount }}</td>
		...
		data() {
			return {
				payment: {
					...
					CPayment_amount: 0,
					CPayment_notes: '',
					CPayment_po_charge_amount: 0,
					CPayment_previous_due: 0,
					CPayment_Previous_due_Po: 0,
				},
				payments: [],
				customers: [],
				selectedCustomer: {
				...
				columns: [{
					...
					{
						label: 'PO Payment',
						field: 'CPayment_po_charge_amount',
						align: 'center'
					},
					{
						label: 'Description',
		...
		methods: {
			...
			getCustomerDue() {
				...
				axios.post('/get_customer_due', {
					...
					this.payment.CPayment_Previous_due_Po = ((parseFloat(res.data[0].POAmount)+parseFloat(res.data[0].salesPoAmount)) -  parseFloat(res.data[0].CustomerPaymentPoReceived)).toFixed(2);
					this.payment.CPayment_previous_due = (parseFloat(res.data[0].dueAmount) - parseFloat(this.payment.CPayment_Previous_due_Po)).toFixed(2);
				})
			},
			getAccounts() {
			...
			saveCustomerPayment() {
				if (this.payment.CPayment_amount == 0 && this.payment.CPayment_po_charge_amount == 0) {
					alert("Must be filed out sales payment or po payment")
					return
				}
			...
			resetForm() {
				...
				this.payment.CPayment_amount = '';
				this.payment.CPayment_amount = '';
				this.payment.CPayment_Previous_due_Po = 0;
				this.payment.CPayment_po_charge_amount = 0;
