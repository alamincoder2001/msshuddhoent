...
class Model_Table extends CI_Model
{
    ...
    public function getTransactionSummary($date = null)
    {
        $transactionSummary = $this->db->query("
            ...
            (
                select ifnull(sum(cp.CPayment_amount + cp.CPayment_po_charge_amount), 0) from tbl_customer_payment cp
    ...
    public function customerDue($clauses = "", $date = null)
    {
        ...
        $dueResult = $this->db->query("
            ...
            (select ifnull(sum(sm.SaleMaster_TotalSaleAmount), 0.00) + ifnull(c.previous_due, 0.00)
                ...
                " . ($date == null ? "" : " and sm.SaleMaster_SaleDate < '$date'") . "
                and sm.Status = 'a') as salesAmount,

            (select ifnull(sum(sm.po_charge), 0.00) + ifnull(c.previous_due, 0.00)
                from tbl_salesmaster sm 
                where sm.SalseCustomer_IDNo = c.Customer_SlNo
                " . ($date == null ? "" : " and sm.SaleMaster_SaleDate < '$date'") . "
                and sm.Status = 'a') as salesPoAmount,
            ...
            (select ifnull(sum(cp.CPayment_po_charge_amount), 0.00) 
                from tbl_customer_payment cp 
                where cp.CPayment_customerID = c.Customer_SlNo 
                and cp.CPayment_TransactionType = 'CR'
                " . ($date == null ? "" : " and cp.CPayment_date < '$date'") . "
                and cp.CPayment_status = 'a') as CustomerPaymentPoReceived,
            ...
            (select ifnull(sum(cpo.po_charge_amount), 0.00) 
                from tbl_customer_po cpo 
                where cpo.customerID = c.Customer_SlNo 
                and cpo.status = 'a'
                " . ($date == null ? "" : " and cpo.date < '$date'") . "
                ) as POAmount,
            ...
            (select invoicePaid + cashReceived + CustomerPaymentPoReceived) as paidAmount,

            (select (salesAmount + paidOutAmount + POAmount) - (paidAmount + returnedAmount)) as dueAmount
