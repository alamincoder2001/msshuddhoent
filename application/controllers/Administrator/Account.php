...
class Account extends CI_Controller
{
    ...
    function getOtherIncomeExpense()
    {
        ...
        if (isset($data->dateFrom) && $data->dateFrom != '' && isset($data->dateTo) && $data->dateTo != '') {
            ...
            $salemasterClause = " and sm.SaleMaster_SaleDate between '$data->dateFrom' and '$data->dateTo'";
            $customerPOClause = " and cpo.Date between '$data->dateFrom' and '$data->dateTo'";
        }

        $result = $this->db->query("
            select
            (
                select ifnull(sum(sm.SaleMaster_Freight), 0)
                from tbl_salesmaster sm
                where sm.Status = 'a'
                and sm.SaleMaster_branchid = '" . $this->session->userdata('BRANCHid') . "'
                $salemasterClause
            ) as sale_transport_cost,

            (
                select ifnull(sum(sm.po_charge), 0)
                from tbl_salesmaster sm
                where sm.Status = 'a'
                and sm.SaleMaster_branchid = '" . $this->session->userdata('BRANCHid') . "'
                $salemasterClause
            ...
            (
                select ifnull(sum(cpo.po_charge_amount), 0)
                from tbl_customer_po cpo
                where cpo.status = 'a'
                and cpo.brunch_id = '" . $this->session->userdata('BRANCHid') . "'
                $customerPOClause
            ) as customer_po_charge,
            ...
            (select po_charge_sale + customer_po_charge) as po_charge
        ")->row();
    ...
    public function getCashLedger()
    {
        ...
        $ledger = $this->db->query("
            ...
            select 
                ...
                (sm.SaleMaster_PaidAmount + sm.po_charge) as in_amount,
                0.00 as out_amount
            from tbl_salesmaster sm 
            ...
            select 
                ...
                (cp.CPayment_amount+cp.CPayment_po_charge_amount) as in_amount,
                0.00 as out_amount
            from tbl_customer_payment cp
            join tbl_customer c on c.Customer_SlNo = cp.CPayment_customerID
