...
class Customer extends CI_Controller
{
    ...
    function addCustomerPO()
    {
        ...
        $data['title'] = 'Add Customer PO';
        $data['content'] = $this->load->view('Administrator/due_report/add_customer_po', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }

    function saveCustomerPO()
    {
        $data = json_decode($this->input->raw_input_stream);
        ...
        try {
            ...
            $payment['brunch_id'] = $this->session->userdata("BRANCHid");

            $this->db->insert('tbl_customer_po', $payment);

            $res = ['success' => true, 'message' => 'Data save successfully'];
        } catch (\Throwable $th) {
            $res = ['success' => false, 'message' => 'Data save failed'];
        }
        echo json_encode($res);
    }
    ...
    function update_customer_po()
    {
        ...
        try {
            ...
            $this->db->where('PO_id', $POId)->update('tbl_customer_po', $payment);
    ...
    function getCustomerPO()
    {
        ...
        if (isset($data->customerId) && $data->customerId != '') {
            $clauses .= " and cpo.customerID = '$data->customerId' ";
        }
        if(isset($data->dateFrom) && $data->dateFrom != ''){
            $clauses .= "and cpo.date BETWEEN '$data->dateFrom' AND '$data->dateTo'";
        }
        
        $res = $this->db->query("SELECT cpo.*,c.Customer_Name,c.Customer_Code
        FROM tbl_customer_po cpo
        LEFT JOIN tbl_customer c on c.Customer_SlNo = cpo.customerID
        WHERE cpo.status = 'a'
        $clauses 
        and cpo.brunch_id = ? order by cpo.date desc", $this->cbrunch)->result();
        echo json_encode($res);
    }

    public function deleteCustomerPo()
    {
        $res = ['success' => false, 'message' => ''];
        try {
            $data = json_decode($this->input->raw_input_stream);

            $this->db->set(['status' => 'd', 'update_by' => $this->session->userdata("userId"), 'UpdateDate' => date('Y-m-d H:i:s')])->where('PO_id', $data->POId)->update('tbl_customer_po');

            $res = ['success' => true, 'message' => 'PO Payment deleted successfully'];
        } catch (Exception $ex) {
            $res = ['success' => false, 'message' => $ex->getMessage()];
        }
    ...
    public function customerPoReport()
    {
        $access = $this->mt->userAccess();
        if (!$access) {
            redirect(base_url());
        }
        $data['title'] = 'Customer PO Report';
        $data['content'] = $this->load->view('Administrator/reports/customer_po_report', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    function getCustomerLedger()
    {
        $data = json_decode($this->input->raw_input_stream);
        $clauses = "";
        if(!empty($data->salesDue) && isset($data->salesDue)){
            $clauses .=" and cp.CPayment_amount <> '0'";
        }
        $previousDueQuery = $this->db->query("select c.previous_due as previous_due from tbl_customer as c where Customer_SlNo = '$data->customerId'")->row();
        ...
        $payments = $this->db->query("
            select 
                ...
                (sm.SaleMaster_TotalSaleAmount-sm.po_charge) as bill,
                sm.SaleMaster_PaidAmount as paid,
                0.00 as POpaid,
                (sm.SaleMaster_DueAmount-sm.po_charge) as due,
                0.00 as returned,
                0.00 as paid_out,
                0.00 as balance,
                sm.po_charge as cpo_charge
            ...
            select
                ...
                cpo.PO_id as id,
                cpo.date as date,
                concat('Customer PO Charge Add - ', c.Customer_Code, ' - ', c.Customer_Name  
                ) as description,
                0.00 as bill,
                0.00 as paid,
                0.00 as POpaid,
                ...
                0.00 as paid_out,
                0.00 as balance,
                cpo.po_charge_amount as cpo_charge
            from tbl_customer_po cpo
            left join tbl_customer as c on c.Customer_SlNo = cpo.customerID
            where cpo.customerID = '$data->customerId'
            and cpo.status = 'a'
            ...
            select
                ...
                cp.CPayment_po_charge_amount as POpaid,
                0.00 as due,
                0.00 as returned,
                0.00 as paid_out,
                0.00 as balance,
        ...
        foreach ($payments as $key => $payment) {
            ...
            $payment->balance = ($lastBalance + $payment->bill + $payment->cpo_charge + $payment->paid_out) - ($payment->paid + $payment->POpaid + $payment->returned);
        }
    ...
    function getCustomerPaymentPoCharge()
    {
        $data = json_decode($this->input->raw_input_stream);
        $clauses = '';
        ...
        $payments = $this->db->query("SELECT cp.*,ba.account_name,ba.account_number, concat('PO Charge receive by ',cp.CPayment_Paymentby) as description
        from tbl_customer_payment cp
        left join tbl_bank_accounts ba on ba.account_id = cp.account_id
        where cp.CPayment_status = 'a'
        $clauses
        order by cp.CPayment_date desc
        ")->result();

        $result = [];

        foreach ($payments as $key => $payment) {
            $lastBalance = $key == 0 ? $payment->CPayment_po_charge_amount : $payments[$key - 1]->balance;
            if ($key == 0) {
                $payment->balance = $lastBalance;
            } else {
                $payment->balance = $lastBalance + $payment->CPayment_po_charge_amount;
            }
